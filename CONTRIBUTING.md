...
| Native package version ↔ matching CHANGELOG entry | `npm run verify:changelog` (in CI, and re-run in `publish-rust.yml` / `publish-kotlin.yml` / `publish-swift.yml` / ADO `pipeline.yml`). |
