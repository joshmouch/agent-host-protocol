/**
 * Shared helper for the type generators: read the telemetry name enums from a
 * telemetry registry module (any source file whose path ends in
 * `/telemetry/registry.ts` — today the .NET-client-private
 * `clients/dotnet/codegen/telemetry/registry.ts`, or `types/telemetry/registry.ts`
 * if AHP ever specs a shared contract) via ts-morph, returning each name's
...
/** Read the telemetry registry. The project must include a source file whose path ends in `/telemetry/registry.ts`. */
export function readTelemetry(project: Project): TelemetryData {
  const sf = project
    .getSourceFiles()
    .find((f) => f.getFilePath().endsWith('/telemetry/registry.ts'));
  if (!sf) {
    throw new Error('readTelemetry: could not locate a **/telemetry/registry.ts source file in project');
  }
