...
final class ReducersTests: XCTestCase {
    ...
    func testSessionReducerDoesNotMutateTurnsArray() {
        let turn1 = Turn(id: "t1", input: .userMessage(UserMessage(kind: .userMessage, text: "First")), responseParts: [], state: .complete)
        let turn2 = Turn(id: "t2", input: .userMessage(UserMessage(kind: .userMessage, text: "Second")), responseParts: [], state: .complete)
        let turn3 = Turn(id: "t3", input: .userMessage(UserMessage(kind: .userMessage, text: "Third")), responseParts: [], state: .complete)
    ...
    func testClientDispatchableReturnsTrue() {
        let action: StateAction = .sessionTurnStarted(SessionTurnStartedAction(
            type: .sessionTurnStarted, turnId: T, input: .userMessage(UserMessage(kind: .userMessage, text: "Hello"))
        ))
