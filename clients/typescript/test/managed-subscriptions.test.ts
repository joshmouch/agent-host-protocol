...
test('coalesces holders and unsubscribes only after the final release', async () => {
  ...
  const first = manager.acquire(URI, 'SessionView');
  const second = manager.acquire(URI, 'SessionView');
