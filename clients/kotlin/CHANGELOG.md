...
- `AhpCommands.ping(id)` request factory for protocol-level connection liveness, mirroring the TypeScript client's `AhpClient.ping()`.
...
- Optional `reviewed` field on `ChangesetFile`. Omitting it (or setting it to
  `null`) signals that the server does not support the file "review"
  functionality.
