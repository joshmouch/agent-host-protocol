...
| --- | --- | --- |
| `lock` / `Monitor` | exclusive lock | **Used** — every exclusive critical section (`HostEntry` field-bundle, channel-list append, subscription registry, timestamp stamp). Cannot be held across `await`. |
| `System.Threading.Lock` (.NET 9 / C# 13) | exclusive lock | No — ~25% faster than `Monitor` *under contention*, but ours are near-zero-contention (see below), and it would require multi-targeting net9, now out of support. |
