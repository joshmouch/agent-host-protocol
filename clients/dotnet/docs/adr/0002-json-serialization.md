# ADR 0002 — JSON serialization engine
...
| --- | --- | --- | --- | --- | --- | --- |
| **System.Text.Json (POCO)** ✅ | Highest | Lowest (`Span<T>`/UTF-8) | Eager | **In-box** (net8/net9) | Source-gen capable | Strict by default; Microsoft's greenfield recommendation. |
| System.Text.Json + source generation | Highest (+startup, +AOT) | Lowest | Eager | In-box | **Best** | A `JsonSerializerContext` enhancement we can add later for AOT/trimming without changing the public API. |
...
- **Throughput + memory:** STJ is the fastest, lowest-allocation option — it is
  ...
  path.
- **Dependencies:** STJ is **in the shared framework** for net8/net9 — the
  packages stay at **zero NuGet dependencies**, which is a hard goal for this
  library.
- **AOT / trimming:** STJ supports source generation, giving us a clean,
  non-breaking path to Native-AOT/trimming friendliness later.
...
- **Source generation:** add a `JsonSerializerContext` for AOT/trimming and a
  further perf bump when there is a concrete AOT consumer.
