...
public sealed class ClientIdStoreTests
{
    ...
    public async Task InMemoryClientIdStore_RoundTrips()
    {
        ...
        await store.StoreAsync(new HostId("h1"), "cid-1", TestContext.Current.CancellationToken);
        ...
        Assert.Null(await store.LoadAsync(new HostId("never-stored"), TestContext.Current.CancellationToken));
    }
    ...
    public async Task InMemoryClientIdStore_Overwrites()
    {
        ...
        await store.StoreAsync(host, "cid-1", TestContext.Current.CancellationToken);
        await store.StoreAsync(host, "cid-2", TestContext.Current.CancellationToken);

        // The second store for the same host wins; reads see the latest value.
        Assert.Equal("cid-2", await store.LoadAsync(host, TestContext.Current.CancellationToken));
    }
