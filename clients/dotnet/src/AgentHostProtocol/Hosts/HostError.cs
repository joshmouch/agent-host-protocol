...
/// <summary>
/// Surfaced on <see cref="HostState.Error"/> when a host enters its terminal
/// <see cref="HostStateKind.Failed"/> state because reconnection cannot proceed:
/// either the transport dropped while the reconnect policy is disabled, or the
/// policy's maximum attempt count was exhausted. Being <see cref="HostException"/>
/// -derived, it is pattern-matchable (<c>state.Error is HostReconnectFailedException</c>)
/// rather than an opaque <see cref="Exception"/>.
/// </summary>
public sealed class HostReconnectFailedException : HostException
{
    /// <summary>Creates a host-reconnect-failed exception with a specific reason message.</summary>
    public HostReconnectFailedException(HostId hostId, string message)
        : base(hostId, message) { }
}
