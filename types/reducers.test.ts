...
describe('isClientDispatchable', () => {
  it('returns true for client-dispatchable actions', () => {
    const action = { type: ActionType.SessionTurnStarted, session: 'x', turnId: 't', input: { kind: MessageKind.UserMessage, text: 'Hello' } } as const;
...
describe('reducer immutability', () => {
  ...
  it('sessionReducer does not mutate original turns array', () => {
    const turn1 = { id: 't1', input: { kind: MessageKind.UserMessage, text: 'First' }, responseParts: [], usage: undefined, state: TurnState.Complete };
    const turn2 = { id: 't2', input: { kind: MessageKind.UserMessage, text: 'Second' }, responseParts: [], usage: undefined, state: TurnState.Complete };
    const turn3 = { id: 't3', input: { kind: MessageKind.UserMessage, text: 'Third' }, responseParts: [], usage: undefined, state: TurnState.Complete };
