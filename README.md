...
- **TypeScript** — Install `@microsoft/agent-host-protocol` to use the wire types, reducers, `AhpClient`, and the `WebSocketTransport`. See [`clients/typescript/`](clients/typescript/) and [`CHANGELOG`](clients/typescript/CHANGELOG.md). Published via the Azure DevOps pipeline at [`clients/typescript/pipeline.yml`](clients/typescript/pipeline.yml) (manual `publishPackage` trigger; no git tag).
